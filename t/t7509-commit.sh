...
test_expect_success '-C option copies only the message with --reset-author' '
	echo "Test 2" >>foo &&
	test_tick &&
	git commit -b -a -C Initial --reset-author &&
...
test_expect_success '-c option copies authorship and message' '
	echo "Test 3" >>foo &&
	test_tick &&
	EDITOR=: VISUAL=: git commit -b -a -c Initial &&
	author_header Initial >expect &&
	author_header HEAD >actual &&
	test_cmp expect actual
'
...
test_expect_success '--amend option with empty author' '
	...
	test_tick &&
	test_must_fail git commit -b -a -m "empty author" --amend 2>err &&
...
test_expect_success '--amend option with missing author' '
	...
	test_tick &&
	test_must_fail git commit -b -a -m "malformed author" --amend 2>err &&
	grep "empty ident" err
'
