...
if [ ! -f ../../../git/contrib/completion/git-completion.bash ]; then
  skip_all="Couldn't find git-completion.bash; skipping these tests"
  test_done
fi
. ../../../git/contrib/completion/git-completion.bash
. ../../bash-completion-eg.sh
...
run_completion ()
{
	...
	(( _cword = ${#_words[@]} - 1 ))
	__git_wrap__eg_main && print_comp
}
...
test_expect_success 'basic' '
	run_completion "git " &&
	# a command we modified
	grep -q "^diff \$" out &&
	# a command overridden in eg just to tweak the help message
	grep -q "^add \$" out &&
	# a command not modified at all by eg
	grep -q "^bisect \$" out &&
	# a command unique to eg
	grep -q "^squash \$" out &&
	# some plumbing command we do not want completion for
	! grep -q "^ls-files \$" out &&
...
test_expect_success 'general options plus command' '
	...
	test_completion "git --exec-path=foo checkout" "checkout " &&
	test_completion "git --html-path checkout" "checkout " &&
...
test_expect_success 'git --help completion' '
	...
	test_completion "git help top" "topic "
	test_completion "git help topic middle-of-b" "middle-of-bisect "
'
