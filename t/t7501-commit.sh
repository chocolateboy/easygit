...
test_expect_success 'amend commit' '
	...
	EOF
	chmod 755 editor &&
	EDITOR=./editor git commit -b --amend
'
...
test_expect_success 'amend --only ignores staged contents' '
	...
	echo changed >file &&
	git add file &&
	git commit -b --no-edit --amend --only &&
...
test_expect_success 'amend without launching editor' '
	...
	git add file &&
	EDITOR=./editor git commit -b --no-edit --amend &&
...
test_expect_success '--amend --edit of empty message' '
	...
	git add file &&
	EDITOR=./replace git commit -b --edit --amend &&
	git diff-tree -s --format=%s HEAD >msg &&
	./replace expect &&
	test_cmp expect msg
'
...
test_expect_success 'editor not invoked if -F is given' '
	...
	EDITOR=./editor git commit -b -a -F msg &&
	git show -s --pretty=format:%s >subject &&
	grep -q good subject &&

	echo quack >file &&
	echo Another good message. |
	EDITOR=./editor git commit -b -a -F - &&
	git show -s --pretty=format:%s >subject &&
	grep -q good subject
'
