...
test_expect_success 'ignore-blank-lines: only new lines' '
	...
	test_seq 5 | sed "/3/i\\
" >x &&
	git diff --unstaged --ignore-blank-lines >out &&
	>expect &&
	test_cmp out expect
'
...
test_expect_success 'ignore-blank-lines: only new lines with space' '
	...
	test_seq 5 | sed "/3/i\\
 " >x &&
	git diff --unstaged -w --ignore-blank-lines >out &&
	>expect &&
	test_cmp out expect
'
...
test_expect_success 'ignore-blank-lines: after change' '
	...
	7
	EOF
	git diff --unstaged --inter-hunk-context=100 --ignore-blank-lines >out.tmp &&
	cat <<-\EOF >expected &&
	diff --git a/x b/x
	--- a/x
	+++ b/x
...
test_expect_success 'ignore-blank-lines: between changes' '
	...
	change
	EOF
	git diff --unstaged --ignore-blank-lines >out.tmp &&
	cat <<-\EOF >expected &&
	diff --git a/x b/x
	--- a/x
	+++ b/x
...
test_expect_success 'ignore-blank-lines: between changes (with interhunkctx)' '
	...
	change
	EOF
	git diff --unstaged --inter-hunk-context=2 --ignore-blank-lines >out.tmp &&
	cat <<-\EOF >expected &&
	diff --git a/x b/x
	--- a/x
	+++ b/x
...
test_expect_success 'ignore-blank-lines: scattered spaces' '
	...
	change
	EOF
	git diff --unstaged --inter-hunk-context=4 --ignore-blank-lines >out.tmp &&
	cat <<-\EOF >expected &&
	diff --git a/x b/x
	--- a/x
	+++ b/x
