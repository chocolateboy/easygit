...
test_expect_success 'stop on conflicting pick' '
	...
	test "$(git diff --unstaged --name-status |
...
test_expect_success '--continue tries to commit, even for "edit"' '
	...
	git add file7 &&
	test_tick &&
	git commit -b -m "unrelated change" &&
...
test_expect_success 'rebase -i with --strategy and -X' '
	...
	echo five >conflict &&
	echo Z >file1 &&
	git commit -b -a -m "one file conflict" &&
