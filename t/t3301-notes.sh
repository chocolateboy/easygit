...
test_expect_success 'git log --pretty=raw does not show notes' '
	git log -1 --pretty=raw | log_cleanup >output &&
	test_cmp expect output
'
...
test_expect_success 'git log --show-notes' '
	git log -1 --pretty=raw --show-notes | log_cleanup >output &&
	test_cmp expect output
'
...
test_expect_success 'Do show note when ref is given in GIT_NOTES_REF' '
	GIT_NOTES_REF="refs/notes/other" git log -1 | log_cleanup > output &&
...
test_expect_success 'Do not show note when core.notesRef is overridden' '
	GIT_NOTES_REF="refs/notes/wrong" git log -1 | log_cleanup > output &&
	test_cmp expect-not-other output
'
...
test_expect_success 'GIT_NOTES_DISPLAY_REF overrides config' '
	git config notes.displayRef "refs/notes/*" &&
	GIT_NOTES_REF= GIT_NOTES_DISPLAY_REF= git log -2 | log_cleanup > output &&
	test_cmp expect-none output
'

test_expect_success '--show-notes=* adds to GIT_NOTES_DISPLAY_REF' '
	GIT_NOTES_REF= GIT_NOTES_DISPLAY_REF= git log --show-notes=* -2 | log_cleanup > output &&
	test_cmp expect-both output
'
...
test_expect_success '--no-standard-notes' '
	git log --no-standard-notes --show-notes=commits -1 | log_cleanup > output &&
	test_cmp expect-commits output
'

test_expect_success '--standard-notes' '
	git log --no-standard-notes --show-notes=commits \
		--standard-notes -2 | log_cleanup > output &&
	test_cmp expect-both output
'

test_expect_success '--show-notes=ref accumulates' '
	git log --show-notes=other --show-notes=commits \
		 --no-standard-notes -1 | log_cleanup > output &&
	test_cmp expect-both-reversed output
'
...
test_expect_success 'append to note from other note with "git notes append -C"' '
	...
	git log -1 HEAD^ | log_cleanup > actual &&
	test_cmp expect actual
'
