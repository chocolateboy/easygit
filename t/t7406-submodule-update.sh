...
test_expect_success 'submodule update - update=none in .git/config but --checkout given' '
	(cd super &&
	 ...
	 git diff --unstaged --raw | test_must_fail grep "	submodule" &&
	 (cd submodule &&
...
test_expect_success 'submodule update continues after recursive checkout error' '
	(cd super &&
	 ...
	 (cd submodule &&
	  ...
	  git submodule init &&
	  git commit --staged -m "new_subsubmodule"
	 ) &&
	 git add submodule &&
	 git commit --staged -m "update_submodule" &&
	 (cd submodule &&
	  (cd subsubmodule &&
	 ...
	 git add submodule &&
	 git add submodule2 &&
	 git commit --staged -m "new_commits" &&
	 git checkout HEAD^ &&
	 (cd submodule &&
	  git checkout HEAD^ &&
	  (cd subsubmodule &&
...
test_expect_success 'submodule update exit immediately after recursive rebase error' '
	(cd super &&
	 ...
	 git add submodule &&
	 git add submodule2 &&
	 git commit --staged -m "two_new_submodule_commits" &&
	 (cd submodule &&
	  git checkout master &&
...
test_expect_success 'submodule update places git-dir in superprojects git-dir' '
	(cd super &&
	 git commit --staged -m "added submodule"
	) &&
...
test_expect_success 'submodule add places git-dir in superprojects git-dir recursive' '
	(cd super2 &&
	 ...
	 git commit -b -m "update submodule" &&
	 git push origin : &&
	 test_cmp actual expected
	)
'
