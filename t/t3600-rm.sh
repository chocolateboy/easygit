...
test_expect_success 'setup subsubmodule' '
	...
	(cd submod &&
		...
		git submodule init &&
		git add .gitmodules &&
		git commit --staged -m "add subsubmodule" &&
		git submodule update subsubmod
	) &&
	git commit -b -a -m "added deep submodule"
'
...
test_expect_success SYMLINKS 'rm across a symlinked leading path (no index)' '
	...
	ln -s e d &&
	git add -A e d &&
	git commit --staged -m "symlink d to e, e/f exists" &&
...
test_expect_success 'rm file with local modification' '
	...
	EOF
	git commit --staged -m "testing rm 3" &&
