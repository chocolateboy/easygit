...
middle-of-am - How to resolve or abort an incomplete C<am> (apply mail)

=head1 DESCRIPTION

When conflicting changes are detected, an C<am> operation will stop to
...
=item 3) Tell C<git> to complete the operation
...
Between the C<E<lt>>s and the C<=>s will be one version
of the changed file, while betwen the C<=>s and the C<E<gt>>s will be another version.

=head3 2. Simple tip

Since C<git> will stage any changes it is able to successfully merge, you can
...
Sometimes, during an C<am>, the changes in a commit will no longer be
...
show that there are no changes at all, and you can continue by telling C<git>
...
symlink etc.), the internal-object-name is C<git>'s internal name for the
...
to tell C<git> that you have resolved the conflicts in FILE.
...
=head2 Tell C<git> to continue the operation
...
to manually insert a new commit; if you already accidentally ran C<eg commit>,
